...
(function () {
    ...
    function videocdn(component, _object) {
      ...
      this.search = function (_object, data) {
        ...
        network.silent(url, function (found) {
          ...
          if (!results.length) component.empty('РџРѕ Р·Р°РїСЂРѕСЃСѓ (' + select_title + ') РЅРµС‚ СЂРµР·СѓР»СЊС‚Р°С‚РѕРІ');
        }, function (a, c) {
      ...
      function extractData(results) {
        ...
        if (movie) {
          var src = movie.iframe_src.replace(/[0-9]{1,9}.svetacdn.in/, 'cors.arkmv.ru/cdn.svetacdn.in');
    ...
    function rezka(component, _object) {
      ...
      function getFirstTranlate(id, call) {
        ...
        network.silent(embed + 'embed/' + id, function (str) {
          extractData(str);
          if (extract.voice.length) call(extract.voice[choice.voice].token);else component.empty('РџРѕ Р·Р°РїСЂРѕСЃСѓ (' + select_title + ') РЅРµС‚ СЂРµР·СѓР»СЊС‚Р°С‚РѕРІ');
        }, function (a, c) {
          component.empty(network.errorDecode(a, c));
        }, false, {
          dataType: 'text'
    ...
    function kinobase(component, _object) {
      ...
      function filtred() {
        ...
        if (object.movie.number_of_seasons) {
          ...
          extract.forEach(function (elem) {
            ...
            if (!elem.title) elem.title = elem.comment || (voice ? voice[1] : 'Р‘РµР· РЅР°Р·РІР°РЅРёСЏ');
      ...
      function extractData(str, page) {
        ...
        if (vod[0] == 'file') {
          ...
          extract = found;
        } else if (vod[0] == 'pl') extract = Lampa.Arrays.decodeJson(vod[1], []);else component.empty('РџРѕ Р·Р°РїСЂРѕСЃСѓ (' + select_title + ') РЅРµС‚ СЂРµР·СѓР»СЊС‚Р°С‚РѕРІ');
      }
    ...
    function collaps(component, _object) {
      var network = new Lampa.Reguest();
      var extract = {};
      var embed = 'http://api.tobaco.ws/embed/';
      var object = _object;
      var select_title = '';
      var filter_items = {};
      var choice = {
    ...
    function cdnmovies(component, _object) {
      ...
      this.search = function (_object, kp_id) {
        ...
        network.silent(url, function (str) {
          ...
          if (iframe && iframe[1]) {
            ...
            component.empty('РџРѕ Р·Р°РїСЂРѕСЃСѓ (' + select_title + ') РЅРµС‚ СЂРµР·СѓР»СЊС‚Р°С‚РѕРІ');
          }
        }, function (a, c) {
          component.empty(network.errorDecode(a, c));
        }, false, {
      ...
      function append(items) {
        ...
        var viewed = Lampa.Storage.cache('online_view', 5000, []);
        items.forEach(function (element) {
          if (element.season) element.title = 'S' + element.season + ' / РЎРµСЂРёСЏ ' + element.episode;
    ...
    function filmix(component, _object) {
      ...
      function extractData(data) {
        ...
        var rip = data.rip.replace(/[0-9]/g,"").slice(0, -1);
    ...
    function component(object) {
      ...
      var last_bls = Lampa.Storage.cache('streamv1_last_balanser', 200, {});
      ...
      var filter_sources = ['videocdn', 'rezka', 'kinobase', 'collaps', 'cdnmovies', 'filmix']; // С€Р°Р»РѕРІР»РёРІС‹Рµ СЂСѓС‡РєРё
      ...
      this.create = function () {
        ...
        filter.onSelect = function (type, a, b) {
          if (type == 'filter') {
            ...
            Lampa.Storage.set('streamv1_last_balanser', last_bls);
      ...
      this.find = function () {
            ...
            network.silent('http://arkmv.ru/kp/' + encodeURIComponent(query), function (json) {
              json.data = json.films;
                  if (json.data.length == '0') {
                        _this2.empty('РџРѕ Р·Р°РїСЂРѕСЃСѓ (' + query + ') РЅРµС‚ СЂРµР·СѓР»СЊС‚Р°С‚РѕРІ');
                  return;
                  }
              display(json);
            }, function (a, c) {
        ...
        if (object.movie.imdb_id) {
          ...
          network.silent('http://i.arkmv.ru/tmdb/3/' + (object.movie.name ? 'tv' : 'movie') + '/' + object.movie.id + '/external_ids?api_key=4ef0d7355d9ffb5151e987764708ce96&language=ru', function (ttid) {
            letgo(ttid.imdb_id);
          }, function (a, c) {
            _this2.empty(network.errorDecode(a, c));
          });
    ...
    function showStatus() {
      ...
      var info = 'РЈСЃС‚СЂРѕР№СЃС‚РІРѕ РЅРµ Р°РІС‚РѕСЂРёР·РѕРІР°РЅРѕ';

      if (status.login) {
        if (status.is_pro) info = status.login + ' - PRO РґРѕ - ' + status.pro_date;else if (status.is_pro_plus) info = status.login + ' - PRO_PLUS РґРѕ - ' + status.pro_date;else info = status.login + ' - NO PRO';
      }
